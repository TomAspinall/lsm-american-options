...
def discount(r, t=1): return exp(-r*t)
...
def LSM_american_option(state_variables,
    ...
    # Assertions:
    assert len(K) > 1 and len(
        K) != number_simulations, "length of object 'K' does not equal 1 or number of columns of 'state_variables'!"
    assert state_variables.notna().all(
    ), "NA's have been specified within 'state_variables'!"
    assert (len(state_variables) != len(payoff)
            ), "Dimensions of object 'state_variables' does not match 'payoff'!"
    ...
    for t in range(number_periods, 0, -1):
        ...
        american_option_value[~exercise] < - \
            american_option_value[~exercise] * discount(r)
    ...
    option_values[exercised_paths] < - profit[number_periods *
                                              (in_the_money_paths-1) + exercise_period] * discount(r, exercise_period)
